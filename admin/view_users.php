...
$db = new mysqli($servername, $username, $password, $database, $port);
        ...
        <main class="content px-3 py-4">
          <div class="container-fluid">
            <div class="mb-3">
              <h3 class="fw-bold fs-4 mb-3">Admin Dashboard (Users)</h3>
              <div class="row g-2">
                  ...
                  <div class="card border-0 bg-primary text-white">
                    <div class="card-body py-4">
                      ...
                      <p class="mb-2 fw-bold">
                        <?php
                        $sql_qyery_all = "SELECT * FROM users;";
                        $result_all = $db -> query($sql_qyery_all);
                        if (!$result_all) {
                          die("Select query error".$db -> error);
                        }
                        $sql_qyery_new = "SELECT * FROM users WHERE status = 'new';";
                        $result_new = $db -> query($sql_qyery_new);
                        if (!$result_new) {
                          die("Select query error".$db -> error);
                        }

                        echo $result_all->num_rows."  (".$result_new -> num_rows." pending for action)";
                        ?>
                      </p>
                    </div>
                  </div>
                ...
                <div class="col-12 col-md-3">
                  <div class="card border-0 bg-warning text-white">
                    <div class="card-body py-4">
                      <h5 class="mb-2 fw-bold">Under verification</h5>
                      <p class="mb-2 fw-bold"><?php
                        $sql_qyery_verification = "SELECT * FROM users WHERE status='under verification';";
                        $result_verification = $db -> query($sql_qyery_verification);
                        if (!$result_verification) {
                          die("Select query error".$db -> error);
                        }

                        echo $result_verification->num_rows;
                        ?></p>
                    </div>
                  </div>
                </div>
                <div class="col-12 col-md-3">
                  <div class="card border-0 bg-success text-white">
                    <div class="card-body py-4">
                      ...
                      <p class="mb-2 fw-bold"><?php
                        $sql_qyery_verified = "SELECT * FROM users WHERE status='verified';";
                        $result_verified = $db -> query($sql_qyery_verified);
                        if (!$result_verified) {
                          die("Select query error".$db -> error);
                        }

                        echo"$result_verified->num_rows";
                        ?></p>
                    </div>
                  </div>
                </div>
                <div class="col-12 col-md-3">
                  <div class="card border-0 bg-danger text-white">
                    <div class="card-body py-4">
                      ...
                      <p class="mb-2 fw-bold">
                        <?php
                        $sql_qyery_rejected = "SELECT * FROM users WHERE status='rejected';";
              ...
              <div class="row">
                <div class="col-12 table-responsive">
                  <table class="table table-hover table-bordered text-nowrap">
                    <thead class="table-dark align-top text-center">
                      <tr class="highlight">
                        ...
                        <th colspan="2" class="col-2">Status</th><th colspan="2" class="col-2">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php
                      $sql_qyery = "SELECT * FROM users";
                          ...
                          <tr>
                            ...
                            <form action="update_user_status.php" method="post">
                                ...
                                <select name="status" id="status" class="form-control">
                                    ...
                                    <option value="under verification" <?= $row["status"] == 'under verification' ? 'selected' : '' ?>>under verification</option>
                            ...
                            <td><button type="button" class="btn btn-warning"><a class="nav-link" href="update_user_details_form.php?user_id=<?= $row['user_id'] ?>">Update details</a></button></td>
                            <td><button type="button" class="btn btn-danger"><a class="nav-link" href="delete_user.php?user_id=<?= $row['user_id'] ?>">Delete</a></button></td>
                          </tr>
                          <?php
                        }
                      }
